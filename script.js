...
async function saveNoteToDB(note) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (note.audioBlob) audioStore.put({ id: note.id, blob: note.audioBlob });
...
async function saveState() {
  ...
  for (const note of state.notes) await saveNoteToDB(note);
}
async function loadState() {
  try {
    ...
    state.backupEnabled =
      settings.backupEnabled !== undefined ? settings.backupEnabled : true;
...
function scheduleBackup() {
  ...
  state.backupInterval = setInterval(
    () => {
      if (state.backupEnabled && state.notes.length > 0) silentBackup();
    },
    10 * 60 * 1000,
  );
}
...
function showScreen(id, direction = "forward") {
  ...
  if (id === "screen-home") setTimeout(() => renderHome(), 50);
...
function toggleProfileMenu(e) {
  ...
  if (state.isProfileMenuOpen) closeProfileMenu();
...
function openProfileMenu() {
  ...
  if (screenRect.width - right - 256 < 16) right = screenRect.width - 256 - 16;
  menu.style.top = top + "px";
  menu.style.right = right + "px";
  renderProfileHeader();
  document.getElementById("app-version").textContent = `v.${APP_VERSION}`;
  const profileDarkToggle = document.getElementById("profile-dark-toggle");
  if (profileDarkToggle) profileDarkToggle.checked = state.theme === "dark";
  ...
  const rows = menu.querySelectorAll(
    ".profile-menu-row, .profile-header, .profile-bottom-bar",
  );
  rows.forEach((row, index) => {
    ...
    setTimeout(
      () => {
        row.style.transition = `opacity 0.3s var(--ease-out-expo), transform 0.4s var(--spring)`;
        row.style.opacity = "1";
...
function updateSettingsScreen() {
  ...
  if (storageBar) {
    const percentage = Math.min(
      (storageSize / (5 * 1024 * 1024 * 1024)) * 100,
      100,
    );
...
function renderHome() {
  ...
  document
    .getElementById("pill-voice")
    .classList.toggle("active", state.currentFilter === "voice");
  document
    .getElementById("pill-written")
    .classList.toggle("active", state.currentFilter === "written");
  const wb = document.getElementById("written-btn");
  if (state.currentFilter === "written") wb.classList.remove("hidden");
  ...
  const writtenNotesCount = state.notes.filter(
    (n) => n.type === "written",
  ).length;
  const filtered = state.notes.filter((n) => {
    ...
    const text = (
      n.title +
      " " +
      (n.editedBody || n.originalTranscription)
    ).toLowerCase();
    const tagsMatch =
      !state.activeTagFilter ||
      (n.tags && n.tags.includes(state.activeTagFilter));
    const searchMatch =
      !search ||
      text.includes(search) ||
      (n.tags && n.tags.some((t) => ("#" + t).includes(search)));
  ...
  if (renderedNoteCount === 0) {
    if (state.currentFilter === "voice" && voiceNotesCount === 0) {
      ...
      tile.innerHTML = `<div class="home-tile-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg></div><div class="empty-text-wrap"><div class="empty-title">Tap + to record</div><div class="empty-sub">Your first voice note</div></div>`;
      ...
      const tile = document.createElement("div");
      tile.className = "home-tile empty empty-full";
      tile.innerHTML = `<div class="home-tile-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg></div><div class="empty-text-wrap"><div class="empty-title">Your first note</div><div class="empty-sub">Tap to start writing</div></div>`;
...
function createTile(note, delay = 0) {
  ...
  const cardClass =
    note.type === "voice" ? "voice-note-card" : "written-note-card";
  tile.classList.add(cardClass);
  const dividerHTML =
    note.type === "voice"
      ? `<div class="note-dots-row">${Array(32)
          .fill(0)
          .map(() => '<div class="progress-dot"></div>')
          .join("")}</div>`
      : `<div class="note-divider-line"></div>`;
  const playPauseBtnHTML =
    note.type === "voice"
      ? `<button class="note-play-pause-btn" onclick="event.stopPropagation(); toggleVoiceNoteAudio('${note.id}', this.closest('.home-tile'))"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></button>`
  ...
  if (note.tags && note.tags.length > 0)
    tagsHTML =
      ...
      note.tags
        ...
        .join("") +
      "</div>";
  tile.innerHTML = `<div class="note-content-wrapper"><div class="note-content">${escapeHtml(cleanBody)}</div></div>${dividerHTML}<div class="note-bottom"><div class="note-title">${escapeHtml(note.title || "Untitled")}</div><div class="note-meta-right">${playPauseBtnHTML}<div class="note-date">${dateStr}</div></div></div>${tagsHTML}`;
...
function toggleVoiceNoteAudio(noteId, tile) {
  ...
  if (
    ...
    state.currentHomeAudio &&
    !state.currentHomeAudio.paused
  ) {
    ...
    if (state.currentHomeAudio) {
      ...
      if (state.currentHomeTile) {
        const prevPlayBtn = state.currentHomeTile.querySelector(
          ".note-play-pause-btn",
        );
        if (prevPlayBtn)
          prevPlayBtn.innerHTML =
            '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>';
        const prevDots =
          state.currentHomeTile.querySelectorAll(".progress-dot");
    ...
    playBtn.innerHTML =
      '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M6 4h4v16H6V4zm8-14v14h4V5h-4z"/></svg>';
    ...
    state.homeAudioInterval = setInterval(() => {
      ...
      dots.forEach((dot, index) => {
        if (index < activeDotsCount) dot.classList.add("active");
...
class AudioReactor {
  constructor(canvas, audioContext, sourceNode) {
    ...
    this.NUM_CAPSULES = 4;
    this.springs = Array(this.NUM_CAPSULES)
      .fill(0)
      .map(() => ({ value: 1, velocity: 0, target: 1 }));
    this.W = 0;
    this.H = 0;
  ...
  getBandValues(freq) {
    ...
    const bands = [
      ...
      { start: Math.floor(len * 0.4), end: Math.floor(len * 0.75) },
      { start: Math.floor(len * 0.06), end: Math.floor(len * 0.25) },
      { start: Math.floor(len * 0.25), end: Math.floor(len * 0.4) },
    ];
  ...
  animate() {
    ...
    if (screen)
      screen.style.setProperty("--audio-energy", overallEnergy.toFixed(3));
    ...
    const dpr = window.devicePixelRatio || 1;
    const baseCapsuleWidth = Math.min(this.W * 0.1, 70 * dpr);
    const gap = baseCapsuleWidth * 0.25;
    const totalWidth =
      this.NUM_CAPSULES * baseCapsuleWidth + (this.NUM_CAPSULES - 1) * gap;
...
async function startRecording() {
  ...
  document.getElementById("rec-live-text").innerHTML =
    '<span class="rec-listening">Listening...</span>';
  ...
  try {
    ...
    state.audioReactor = new AudioReactor(
      recCanvas,
      state.audioContext,
      source,
    );
...
function finishRecording() {
  ...
  state.mediaRecorder.onstop = () => {
    const audioBlob = new Blob(state.audioChunks, {
      type: state.currentMimeType || "audio/webm",
    });
...
function initSpeechRecognition() {
  ...
  state.recognition.onresult = (event) => {
    ...
    for (let i = event.resultIndex; i < event.results.length; i++) {
      ...
      if (result.isFinal) {
        ...
        if (words.length > 0) {
          ...
          const timePerWord =
            words.length > 0 ? chunkDuration / words.length : 0;
    ...
    if (finalAdd) state.finalTranscript += finalAdd;
  ...
  state.recognition.onend = () => {
    ...
    if (state.isRecording && !state.isPaused) {
      try {
        setTimeout(() => {
          if (state.isRecording && !state.isPaused) state.recognition.start();
        }, 150);
      } catch (e) {
  ...
  state.recognition.onspeechend = () => {
    const elapsed = getRecordingElapsedTime();
    if (elapsed - state.lastSentenceEnd > 1.5) state.lastSentenceEnd = elapsed;
  };
}
function formatTranscriptChunk(rawText, elapsedTime) {
  if (!rawText || !rawText.trim())
    return { formatted: "", sentenceEnded: false };
  let text = rawText.trim();
  text = text.replace(/\s+/g, " ");
  if (text.length > 0) text = text.charAt(0).toUpperCase() + text.slice(1);
  if (text.length > 30)
    text = text.replace(
      /\s+(and|but|or|so|yet|however|therefore|meanwhile|although|because|since)\s+/gi,
      (match, conj) => `, ${conj.toLowerCase()}`,
    );
  const introPhrases =
    /^(however|therefore|meanwhile|furthermore|moreover|additionally|consequently|nevertheless|specifically|for example|for instance|in addition|on the other hand|as a result|in conclusion|to summarize|first|second|third|finally|lastly|next|then|after that|before that)\b/i;
    ...
    ctx.fillStyle =
      x < playheadX
        ...
        : isDark
          ...
          : "#D0D0D0";
    ctx.beginPath();
    if (ctx.roundRect) ctx.roundRect(x, y, barWidth, bh, 1);
    else ctx.rect(x, y, barWidth, bh);
    ctx.fill();
  }
  state.scrubTrail = state.scrubTrail.filter(
    (t) => performance.now() - t.timestamp < 150,
  );
  state.scrubTrail.forEach((trail) => {
    const age = (performance.now() - trail.timestamp) / 150;
    const opacity = 0.4 * (1 - age);
    const trailX = trail.progress * dw;
    ctx.fillStyle = `rgba(232, 52, 42, ${opacity})`;
    ctx.beginPath();
    ctx.arc(trailX, cy, 5 * 0.6, 0, Math.PI * 2);
    ctx.fill();
  });
...
function discardReview() {
  showModal(
    ...
    () => {
      stopReviewAudio();
      if (state.currentReviewAudioUrl)
        URL.revokeObjectURL(state.currentReviewAudioUrl);
      goBackFromReview();
      haptic([10, 50, 10]);
    },
    "#FF3B30",
...
async function saveNote() {
  ...
  if (state.currentReviewBlob) {
    ...
    try {
      ...
      const ext =
        state.currentMimeType && state.currentMimeType.includes("mp4")
          ? "mp4"
          : "webm";
...
async function openNoteDetail(id) {
  ...
  const ds = Math.floor(dur % 60)
    .toString()
    .padStart(2, "0");
  document.getElementById("det-audio-length").textContent =
    note.type === "voice" ? `${dm}:${ds}` : "--:--";
  ...
  if (note.type === "voice" && note.audioFileURL) {
    ...
    decodeAudioDuration(note.audioFileURL, note.duration || 1).then(
      (decodedDuration) => {
...
function buildKaraokeMap(note) {
  ...
  const originalWords = originalTimings.map((t) =>
...
function updateWordHighlight() {
  ...
  if (
    activeIndex === -1 &&
    currentKaraokeMap.length > 0 &&
    currentTime >= currentKaraokeMap[currentKaraokeMap.length - 1].end
  )
    activeIndex = currentKaraokeMap[currentKaraokeMap.length - 1].domWordIndex;
  const container = document.getElementById("det-editable-text");
  const activeSpan = container.querySelector(
    `.karaoke-word[data-k-index="${activeIndex}"]`,
  );
  container
    .querySelectorAll(".karaoke-word.det-current-word")
    .forEach((el) => el.classList.remove("det-current-word"));
  if (activeSpan) {
    ...
    if (
      ...
      rect.bottom > containerRect.bottom - 80
    )
...
function renderDetailText() {
  ...
  diffResult.forEach((part) => {
    ...
    tokens.forEach((token) => {
      ...
      span.className =
        "karaoke-word " + (part.added ? "det-added" : "det-original");
...
function getRawTextFromDOM(el) {
  ...
  el.childNodes.forEach((node, index) => {
    if (node.nodeType === Node.TEXT_NODE) raw += node.textContent;
      ...
      } else raw += getRawTextFromDOM(node);
    }
  });
  ...
  function traverse(node) {
    ...
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      if (node.tagName === "BR") {
        ...
        if (node.previousSibling || charCount > 0) isBlockNewline = true;
        if (isBlockNewline) {
          if (charCount === offset) {
...
function drawDetailWaveform(
  ...
  else ctx.rect(lineStartX, cy - lineThickness / 2, lineWidth, lineThickness);
  ctx.fill();
  if (safeProgress > 0) {
  ...
  state.scrubTrail.forEach((trail) => {
    const age = (performance.now() - trail.timestamp) / 150;
    const opacity = 0.4 * (1 - age);
    const trailX = lineStartX + trail.progress * lineWidth;
    ctx.fillStyle = `rgba(232, 52, 42, ${opacity})`;
    ctx.beginPath();
    ctx.arc(trailX, cy, baseRadius * 0.6, 0, Math.PI * 2);
    ctx.fill();
  });
  const radius = baseRadius * playheadScale;
...
async function saveAndGoHome() {
  ...
  if (note) {
    note.title =
      document.getElementById("det-title-input").value.trim() || note.title;
    note.editedBody = getRawTextFromDOM(
      document.getElementById("det-editable-text"),
    );
    stopDetailAudio();
    await saveState();
  } else stopDetailAudio();
  goBack();
}
...
function shareCurrentNote() {
  ...
  else showToast("Share sheet opened");
  haptic(10);
}
  ...
  reader.onload = (e) => {
    try {
      ...
      if (Array.isArray(imported)) {
        imported.forEach((n) => {
          if (n.audioData) {
            try {
              ...
              n.audioBlob = new Blob([byteArray], {
                type: n.mimeType || "audio/webm",
              });
        ...
        haptic([10, 50, 10]);
      } else showToast("Invalid backup format");
    } catch (err) {
...
function measureAndPositionMenu() {
  ...
  if (left < 10) left = 10;
  if (left + menuWidth > screenRect.width - 10)
    left = screenRect.width - menuWidth - 10;
...
function handleGlobalPointerDown(e) {
  ...
  if (editableText.contains(e.target)) {
    setTimeout(() => {
      if (!isMenuOpen) return;
      const sel = window.getSelection();
      if (!sel.rangeCount || sel.isCollapsed) hideDetContextMenu();
      else {
        const range = sel.getRangeAt(0);
        if (!editableText.contains(range.commonAncestorContainer))
          hideDetContextMenu();
        else measureAndPositionMenu();
      }
    }, 10);
  } else {
    hideDetContextMenu();
...
function handleMediaImport(event) {
  ...
  Array.from(files).forEach((file, index) => {
    const id =
      "media_" + Date.now() + "_" + Math.random().toString(36).substr(2, 5);
...
function createMediaElement(m, container) {
  ...
  if (m.type.startsWith("image/"))
    ...
    contentHTML = `<div style="display:flex; flex-direction:column; align-items:center; justify-content:center; width:100%; height:100%; padding:10px; text-align:center; background:rgba(255,255,255,0.95); border: 1px solid #eee;"><svg viewBox="0 0 24 24" width="32" height="32" fill="none" stroke="#888" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg><div style="margin-top:8px; font-size:12px; color:#666; word-break:break-all;">${escapeHtml(m.name)}</div></div>`;
  item.innerHTML = `<div class="media-content">${contentHTML}</div><div class="media-handle media-rotate-handle"><svg viewBox="0 0 24 24"><path d="M21.5 2v6h-6M2.5 22v-6h6M2 11.5a10 10 0 0 1 18.8-4.3M22 12.5a10 10 0 0 1-18.8 4.2"/></svg></div><div class="media-handle media-resize-handle"><svg viewBox="0 0 24 24"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/></svg></div><div class="media-handle media-delete-handle"><svg viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></div>`;
...
function resetMediaHideTimer() {
  ...
  mediaHideTimer = setTimeout(() => {
    document
      .querySelectorAll(".media-item.show-handles")
      .forEach((el) => el.classList.remove("show-handles"));
  }, 2000);
}
...
function hideDeleteModal() {
  ...
  setTimeout(() => {
    ...
    try {
      if (state.deleteModalHistoryPushed && !isHandlingPopstate) history.back();
    } catch (e) {}
...
function initScrubbing() {
  ...
  function updateDetScrub(e) {
    ...
    state.scrubTrail.push({ progress: progress, timestamp: performance.now() });
    if (state.scrubTrail.length > 6) state.scrubTrail.shift();
    const dur = getDetailDuration(note);
  ...
  detContainer.addEventListener("pointerdown", onDetPointerDown, {
    passive: false,
  });
  detContainer.addEventListener(
    "touchstart",
    (e) => {
      e.preventDefault();
  ...
  function onRevPointerUp(e) {
    ...
    if (revWasPlaying && audio) audio.play().catch(() => {});
  }
  revContainer.addEventListener("pointerdown", onRevPointerDown, {
    passive: false,
  });
  revContainer.addEventListener(
    "touchstart",
    (e) => {
      e.preventDefault();
...
function initLiquidPill() {
  ...
  /* ── Spring physics ────────────────────────────────────────────
     Model : a = K*(tgt - pos) - D*vel;  vel += a;  pos += vel
     K = stiffness × dt²,  D = damping × dt   (dt = 1/60 s)
     Tuned : ω₀ ≈ 12 rad/s, ζ ≈ 0.65  →  ~5 % overshoot, ~350 ms
  ──────────────────────────────────────────────────────────────── */
  const K  = 0.040, D  = 0.260;   // position  (snappy, slight bounce)
  const Ks = 0.024, Ds = 0.220;   // size      (a touch softer)

  const sp = {
    x: { pos: 0,  vel: 0, tgt: 0  },
    y: { pos: 0,  vel: 0, tgt: 0  },
    w: { pos: 64, vel: 0, tgt: 64 },
    h: { pos: 64, vel: 0, tgt: 64 },
  };

  function tick(s, k, d) {
    s.vel += (s.tgt - s.pos) * k - s.vel * d;
    s.pos += s.vel;
  }
  function atRest(s, eps = 0.12) {
    return Math.abs(s.tgt - s.pos) < eps && Math.abs(s.vel) < eps;
  }

  let raf = null, ready = false, prevBtn = null;

  function frame() {
    tick(sp.x, K, D); tick(sp.y, K, D);
    tick(sp.w, Ks, Ds); tick(sp.h, Ks, Ds);

    /* Squash & stretch — pill elongates in the direction of travel,
       border-radius naturally turns it into a liquid blob mid-flight */
    const stretch = Math.min(Math.abs(sp.x.vel) * 2.8, sp.w.tgt * 0.5);

    slider.style.transform = `translate(${sp.x.pos - stretch * 0.5}px, ${sp.y.pos}px)`;
    slider.style.width     = `${sp.w.pos + stretch}px`;
    slider.style.height    = `${sp.h.pos}px`;

    if (atRest(sp.x) && atRest(sp.y) && atRest(sp.w) && atRest(sp.h)) {
      // Snap to exact target to eliminate sub-pixel drift
      slider.style.transform = `translate(${sp.x.tgt}px, ${sp.y.tgt}px)`;
      slider.style.width     = `${sp.w.tgt}px`;
      slider.style.height    = `${sp.h.tgt}px`;
      raf = null;
    } else {
  ...
  function getTarget() {
    const btn = container.querySelector(".pill-btn.active");
    if (!btn) return null;
    const cR = container.getBoundingClientRect();
    const bR = btn.getBoundingClientRect();
    const cs = getComputedStyle(container);
    return {
      x: Math.round(bR.left - cR.left - parseFloat(cs.borderLeftWidth)),
      y: Math.round(bR.top  - cR.top  - parseFloat(cs.borderTopWidth)),
      w: Math.round(bR.width),
      h: Math.round(bR.height),
    };
  }

  // Icon pops down then springs back when its button becomes active
  function popIcon(btn) {
    if (!btn || btn === prevBtn) return;
    prevBtn = btn;
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => {
      svg.style.transition = "transform 0.55s var(--spring)";
  ...
  function update(animate = true) {
    ...
    sp.x.tgt = t.x; sp.y.tgt = t.y;
    sp.w.tgt = t.w; sp.h.tgt = t.h;

    if (!animate || !ready) {
      // First render: place instantly, no physics
      sp.x.pos = t.x; sp.x.vel = 0;
      sp.y.pos = t.y; sp.y.vel = 0;
      sp.w.pos = t.w; sp.w.vel = 0;
      sp.h.pos = t.h; sp.h.vel = 0;
      slider.style.transform = `translate(${t.x}px, ${t.y}px)`;
    ...
    raf = requestAnimationFrame(frame);
    popIcon(container.querySelector(".pill-btn.active"));
  }

  requestAnimationFrame(() => update(false));

  new MutationObserver(() => update(true)).observe(container, {
...
async function init() {
  ...
  document
    ...
    .forEach((btn) => {
      btn.addEventListener("pointerdown", (e) => {
        ...
        const scale = 1 - pressure * 0.08;
        btn.style.transition = "transform 0.1s ease-out";
        btn.style.transform = `scale(${scale})`;
      });
      btn.addEventListener("pointerup", () => {
        btn.style.transition = "transform 0.3s var(--spring)";
        btn.style.transform = "";
      });
      btn.addEventListener("pointerleave", () => {
        btn.style.transition = "transform 0.3s var(--spring)";
  ...
  document.addEventListener("pointerdown", (e) => {
    if (!e.target.closest(".media-item")) hideAllMediaHandles();
  });
    ...
    setTimeout(() => {
      ...
      if (sel && sel.rangeCount > 0 && !sel.isCollapsed) {
        ...
        if (detEditText.contains(range.commonAncestorContainer)) {
          if (!isMenuOpen) showDetContextMenu(e);
          else measureAndPositionMenu();
        }
      }
    }, 50);
  ...
  ctxMenu.addEventListener(
    ...
    (e) => {
      const editableText = document.getElementById("det-editable-text");
      if (document.activeElement !== editableText) editableText.focus();
      ensureSelection();
    },
    { passive: true },
  );
  ...
  document
    ...
    .addEventListener("click", (e) => {
      if (e.target.id === "delete-modal-overlay") hideDeleteModal();
    });
  document
